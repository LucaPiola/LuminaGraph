window.STGraphXI18nBundles = {
  "it": {
    ...
    "expr.help.grid": "grid(righe, colonne[, valore]): costruisce una matrice spaziale a partire da coordinate intere non negative. righe indica le righe, colonne indica le colonne. Se valore manca usa 1; se e scalare usa quel valore in tutte le celle occupate; se e un vettore usa valore[i] nella cella di riga righe[i] e colonna colonne[i]. Le altre celle valgono 0. Esempi: grid([1,1], [0,2]) -> [[0,0,0],[1,0,1]] ; grid([1,1], [0,2], [1,2]) -> [[0,0,0],[1,0,2]]. Collisioni sulla stessa cella producono errore.",
  ...
  "en": {
    ...
    "expr.help.grid": "grid(rows, cols[, value]): builds a spatial matrix from non-negative integer coordinates. rows gives the rows, cols gives the columns. If value is omitted it uses 1; if it is scalar it uses that value in every occupied cell; if it is a vector it uses value[i] in the cell at row rows[i] and column cols[i]. All other cells are 0. Examples: grid([1,1], [0,2]) -> [[0,0,0],[1,0,1]] ; grid([1,1], [0,2], [1,2]) -> [[0,0,0],[1,0,2]]. Collisions on the same cell raise an error.",
